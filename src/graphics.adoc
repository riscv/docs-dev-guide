...
While Wavedrom simplifies the creation of accurate SVGs for register diagrams, the graphical elements--those for the various diagrams--add complexity to the build.
...
The following json-formatted script, when added within an AsciiDoc block with the macro indicators `[wavedrom, ,]`, embeds the diagram output into the pdf:
