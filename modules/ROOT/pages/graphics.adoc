...
----
.Figure title
[wavedrom,target="op-32-add-wavedrom",]
....
{reg:[
...
. Wavedrom code for all diagrams in this illustration are stored under `partials/wavedrom`; the generated SVGs land in `images/wavedrom` at build time.
...
[source,adoc]
----
include::partial$wavedrom/filename.adoc
----
====
...
* store scripts for Graphviz diagrams in the examples/graphviz directory, as <filename>.txt
...
----
.Sample litmus test
graphviz::example$graphviz/litmus_sample.txt[align="center"]
...
include::partial$bytefield/examplebyte.adoc[]}
     C_0 -- H_1 [type=s];
     C_0 -- H_2 [type=s];
     C_0 -- C_1 [type=s];
     C_1 -- H_3 [type=s];
